...
export class BattleScene extends Phaser.Scene{
  ...
  bestAbilityAction(u,enemyTeam){const profile=getAIProfile(u.aiProfile);const abilities=this.abilitiesFor(u).filter(a=>u.ap>=a.apCost&&u.mp>=this.abilityMpCost(u,a)&&this.abilityCooldown(u,a)<=0&&!u.usedAbilitiesThisTurn?.includes(a.id)&&!a._aiBlocked);let best=null;for(const a of abilities){const cells=this.skillCells(u,a);let candidates=[];if(a.targetMode==='self')candidates=[null];else candidates=cells;for(const cell of candidates){const t=cell?this.unitAt(cell.col,cell.row):u;let score=0;if(a.targetMode==='frontEmpty'){if(!cell)continue;score=(a.id==='runeEarthWall'?15:35)*profile.summon;}else if(a.targetMode==='ally'){if(!t||t.team!==u.team||t.hp>=t.maxHp)continue;score=(70+(1-t.hp/t.maxHp)*80)*profile.heal;}else if(a.targetMode==='self'){if(['bloodThirst','crimsonPact'].includes(a.id)&&u.hp<=6)continue;if(['miniaturize','maximize','devilBloodThorns','jetBlackRipperBlood','demonicHeart','runeFadingFire'].includes(a.id)&&u.buffs.some(b=>b.id===({miniaturize:'miniaturized',maximize:'maximized',devilBloodThorns:'bloodThorns',jetBlackRipperBlood:'stealth',demonicHeart:'lifesteal',runeFadingFire:'fireDamage'}[a.id])))continue;score=35*profile.buff;}else if(a.targetMode==='cell'){score=30*profile.damage;}else{if(!t||t.team!==enemyTeam)continue;score=85*profile.damage+(t.hp<=10?100*profile.finisher:0);}if(!best||score>best.score)best={ability:a,cell,score};}}return best;}
