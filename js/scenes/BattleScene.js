...
export class BattleScene extends Phaser.Scene{
  ...
  spawnUnit(type,team,col,row,level=1,recruitId=null,scale=null,learnedAbilities=[],blessing=null,options={}){
    ...
    const u={id:this.nextId++,recruitId,type,team,col,row,level,name:options.name||cls?.name||'Zombi',blessing:blessing||cls?.defaultBlessing||null,...stats,hp:stats.maxHp,mp:stats.maxMp,ap:GAME.apPerTurn,maxAp:GAME.apPerTurn,alive:true,learnedAbilities:known,buffs:[],paralyzedTurns:0,turnsTaken:0,blessingBonus:0,isSummon:!!options.isSummon,isObstacle:!!options.isObstacle,resurrectionUsed:false,copiedAbility:null};
    this.units.push(u);return u;
  }
  applyBlessings(){for(const team of ['player','enemy']){const list=this.units.filter(u=>u.team===team&&!u.isSummon&&u.blessing),counts={};list.forEach(u=>counts[u.blessing]=(counts[u.blessing]||0)+1);list.forEach(u=>{const b=BLESSINGS[u.blessing];if(!b)return;const bonus=blessingBonusForCount(counts[u.blessing]||1);u[b.stat]+=bonus;u.blessingBonus=bonus;});}}
  drawUnit(u){
    const{x,y}=this.cellCenter(u.col,u.row),texture=u.isObstacle?'earthWall':u.type;
    u.sprite=this.add.image(x,y,texture).setDisplaySize(u.isObstacle?90:82,u.isObstacle?90:82).setFlipX(u.team==='enemy').setInteractive({useHandCursor:true});if(u.team==='enemy')u.sprite.setTint(0xffdddd);
    ...
    u.hpBack=this.add.rectangle(x,y-48,76,8,0x190d14,.95).setStrokeStyle(1,0xffffff,.35);u.hpBar=this.add.rectangle(x-37,y-48,74,6,0x42d36e,1).setOrigin(0,.5);u.arrow=this.add.image(x,y+48,'arrow').setDisplaySize(25,25).setFlipX(u.team==='enemy').setAlpha(u.isObstacle?0:.72);
  }
  createHud(){
    this.add.rectangle(640,612,1235,205,0x120d1b,.97).setStrokeStyle(3,0x786589);
  ...
  beginTurn(){
    ...
    this.active.ap=this.active.maxAp;this.active.turnsTaken++;this.active.usedAbilitiesThisTurn=[];this.selectedSkillIndex=0;
    ...
    if(this.active.paralyzedTurns>0){this.active.paralyzedTurns--;flashText(this,`${this.active.name} está paralizado.`,640,110,0x91c9ff);this.time.delayedCall(700,()=>this.endTurn());return;}
    if(this.active.team==='enemy'||this.autoBattle){this.infoText.setText(`${this.active.team==='enemy'?'Turno enemigo':'Auto-battle'}: ${this.active.name}`);this.time.delayedCall(400,()=>this.runAIControlled(this.active));}else{this.infoText.setText(`Tu turno: ${this.active.name}`);this.updateButtons();}
  }
  ...
  executeAbility(u,targetCell,a=this.selectedAbility(u)){
    ...
    if(['runeLightningSpear','spitterSpawnBlood'].includes(a.id)){
      if(!target||target.team===u.team)return;this.breakStealth(u);if(a.id==='runeLightningSpear'){const d=Math.max(1,rollDie(5)+Math.floor(u.int/3)-Math.floor(target.df/4))+this.bonusDamage(u);this.dealDamage(u,target,d,'lightningFx',()=>{if(Math.random()<.05)this.addNegativeStatus(target,'paralyzed',1);this.payAbility(u,a);});}else{this.addNegativeStatus(target,'poison',5);flashText(this,'¡Envenenado!',target.sprite.x,target.sprite.y-78,0x83e36e);this.payAbility(u,a);}return;
    }
    ...
    if(a.id==='runeEarthWall'){if(target)return;const w=this.spawnUnit('wall',u.team,targetCell.col,targetCell.row,1,null,null,[],null,{isSummon:true,isObstacle:true,name:'Muro Terrestre'});w.maxHp=w.hp=20;w.df=0;this.drawUnit(w);this.payAbility(u,a);return;}
    ...
    if(a.id==='runeRaiseDead'){if(target)return;const z=this.spawnUnit('zombie',u.team,targetCell.col,targetCell.row,1,null,null,[],null,{isSummon:true,name:'Zombi'});this.drawUnit(z);this.buildTurnQueue();this.payAbility(u,a);return;}
    ...
    if(a.id==='adaptiveResistance'){if(!u.buffs.some(b=>b.id==='adaptiveResistance'))u.buffs.push({id:'adaptiveResistance',remaining:999});this.payAbility(u,a);return;}
    if(a.id==='wateryDodge'){if(!u.buffs.some(b=>b.id==='wateryDodge'))u.buffs.push({id:'wateryDodge',remaining:999});this.payAbility(u,a);return;}
    ...
    if(a.id==='perfectTransformation'){if(!target||target===u)return;const original={type:u.type,name:u.name,str:u.str,int:u.int,df:u.df,agi:u.agi,charisma:u.charisma,will:u.will,stealth:u.stealth,perception:u.perception,learnedAbilities:[...u.learnedAbilities]};u.type=target.type;u.name=target.name;for(const k of ['str','int','df','agi','charisma','will','stealth','perception'])u[k]=target[k];u.learnedAbilities=[...target.learnedAbilities];u.sprite.setTexture(target.type);u.buffs.push({id:'transformed',remaining:3,original});this.payAbility(u,a);return;}
    if(a.id==='imperfectTransformation'){const enemy=u.team==='player'?'enemy':'player',copy=this.lastAbilityByTeam[enemy];if(!copy)return flashText(this,'El rival aún no ha usado una habilidad.',640,120,0xffbd69);u.copiedAbility=copy;u.buffs.push({id:'copiedAbility',remaining:1});this.payAbility(u,a);return;}
    if(a.id==='jetBlackRipperBlood'){this.addBuff(u,{id:'stealth',remaining:2,agi:2});this.payAbility(u,a);return;}
    if(a.id==='bloodThirst'){if(u.hp<=5)return flashText(this,'No tienes suficiente HP.',640,120,0xff7777);u.hp-=5;u.ap+=3;u.str+=1;u.buffs.push({id:'bloodThirst',remaining:1,str:1});this.updateUnitHp(u);this.payAbility(u,a);return;}
  ...
  addBuff(u,b){for(const k of ['str','df','agi','stealth'])if(b[k])u[k]+=b[k];u.buffs.push(b);}
  addNegativeStatus(u,id,duration){if(u.buffs.some(b=>b.id==='adaptiveResistance'))duration=Math.max(1,duration-3);if(id==='paralyzed')u.paralyzedTurns=Math.max(u.paralyzedTurns,duration);else u.buffs.push({id,remaining:duration,negative:true});}
  ...
  payAbility(u,a,refund=0){u.ap=Math.max(0,u.ap-a.apCost+refund);u.mp=Math.max(0,u.mp-a.mpCost);this.afterAction();this.checkEnd();}
  ...
  applyInstantDamage(attacker,target,damage,fx){if(target.buffs.some(b=>b.id==='wateryDodge')){target.buffs=target.buffs.filter(b=>b.id!=='wateryDodge');return;}target.hp-=damage;this.spawnFx(target,fx);floatNumber(this,`-${damage}`,target.sprite.x,target.sprite.y-62,0xff626f);this.updateUnitHp(target);if(target.hp<=0)this.killUnit(target);this.afterDamage(attacker,target,damage);}
  afterDamage(attacker,target,damage){if(attacker?.buffs?.some(b=>b.id==='lifesteal'))this.heal(attacker,Math.max(1,Math.floor(damage*.5)));}
  ...
  endTurn(){if(this.battleOver||!this.active)return;this.tickBuffs(this.active);this.action='none';this.clearHighlights();this.queueIndex++;this.time.delayedCall(180,()=>this.beginTurn());}
  tickBuffs(u){u.buffs.forEach(b=>b.remaining--);const expired=u.buffs.filter(b=>b.remaining<=0);expired.forEach(b=>{for(const k of ['str','df','agi','stealth'])if(b[k])u[k]-=b[k];if(b.scale)u.sprite.setScale(1);if(b.id==='transformed'&&b.original){Object.assign(u,b.original);u.learnedAbilities=[...b.original.learnedAbilities];u.sprite.setTexture(u.type);}if(b.id==='copiedAbility')u.copiedAbility=null;});u.buffs=u.buffs.filter(b=>b.remaining>0);}
  ...
  runAIControlled(u){if(!u?.alive||this.battleOver||u!==this.active)return;const enemyTeam=u.team==='player'?'enemy':'player';const loop=()=>{if(this.battleOver||u!==this.active||!u.alive)return;if(u.ap<=0)return this.endTurn();const abilities=this.abilitiesFor(u).filter(a=>u.ap>=a.apCost&&u.mp>=a.mpCost&&!u.usedAbilitiesThisTurn?.includes(a.id));for(let i=0;i<abilities.length;i++){this.selectedSkillIndex=this.abilitiesFor(u).indexOf(abilities[i]);if(this.aiUseAbility(u,abilities[i],enemyTeam,loop))return;}const front=this.frontTarget(u);if(front&&front.team===enemyTeam&&u.ap>=2)return this.basicAttack(u,front,()=>{u.ap-=2;this.updateHud();if(!this.checkEnd())this.time.delayedCall(180,loop);});const targets=this.units.filter(q=>q.alive&&q.team===enemyTeam&&!q.isObstacle&&!q.buffs.some(b=>b.id==='stealth'&&Math.abs(q.col-u.col)+Math.abs(q.row-u.row)>1)),moves=this.validMoves(u);if(u.ap>=1&&moves.length&&targets.length){const best=moves.map(m=>({m,score:Math.min(...targets.map(t=>Math.abs(t.col-m.col)+Math.abs(t.row-m.row)))})).sort((a,b)=>a.score-b.score)[0].m;return this.moveUnit(u,best.col,best.row,()=>{u.ap--;this.updateHud();this.time.delayedCall(180,loop);});}this.endTurn();};loop();}
  aiUseAbility(u,a,enemyTeam,loop){const old=this.active;this.active=u;const cells=this.skillCells(u,a);let cell=null;if(['self'].includes(a.targetMode))cell=null;else if(a.targetMode==='ally')cell=cells.map(p=>({p,t:this.unitAt(p.col,p.row)})).filter(x=>x.t&&x.t.hp<x.t.maxHp).sort((x,y)=>x.t.hp/x.t.maxHp-y.t.hp/y.t.maxHp)[0]?.p;else if(a.targetMode==='anyUnit')cell=cells.map(p=>({p,t:this.unitAt(p.col,p.row)})).find(x=>x.t?.team===enemyTeam)?.p;else if(a.targetMode==='cell')cell=Phaser.Utils.Array.GetRandom(cells);else cell=cells.find(p=>{const t=this.unitAt(p.col,p.row);return a.targetMode==='frontEmpty'? !t : t?.team===enemyTeam;});if(a.targetMode!=='self'&&!cell){this.active=old;return false;}this.executeAbility(u,cell,a);this.time.delayedCall(220,loop);return true;}
  updateHud(){if(!this.active)return;const portrait=this.active.type==='zombie'?'zombiePortrait':CLASSES[this.active.type]?.portrait||'zombiePortrait';this.portrait.setTexture(portrait);this.nameText.setText(`${this.active.name} · Nv. ${this.active.level}`);this.resourceText.setText(`HP ${Math.max(0,this.active.hp)}/${this.active.maxHp}     MP ${this.active.mp}/${this.active.maxMp}     AP ${this.active.ap}/${this.active.maxAp}`);const arr=this.abilitiesFor(this.active);this.buttons.skills.forEach((b,i)=>{const a=arr[i];b.icon.setTexture(a?.icon||'skill');b.txt.setText(a?`${a.rarity} · ${a.name}`:`SKILL ${i+1}`);b.ct.setText(a?`${a.apCost} AP · ${a.mpCost} MP`:'Bloqueada');});}
  updateButtons(){const can=this.active?.team==='player'&&!this.autoBattle;[this.buttons.move,this.buttons.attack,this.buttons.end,...this.buttons.skills].forEach(b=>{b.bg.setAlpha(can?1:.35);b.icon.setAlpha(can?1:.35);b.icon.clearTint();});if(!can)return;if(this.active.ap<1)this.buttons.move.icon.setTint(0xff5555);if(this.active.ap<2)this.buttons.attack.icon.setTint(0xff5555);const arr=this.abilitiesFor(this.active);this.buttons.skills.forEach((b,i)=>{const a=arr[i];if(!a)b.icon.setTint(0x222222);else if(this.active.ap<a.apCost||this.active.mp<a.mpCost)b.icon.setTint(0xff5555);});}
  showStats(u){
  ...
  killUnit(u){u.alive=false;this.tweens.add({targets:[u.sprite,u.hpBack,u.hpBar,u.arrow],alpha:0,y:u.sprite.y+25,duration:300,onComplete:()=>{[u.sprite,u.hpBack,u.hpBar,u.arrow].forEach(x=>x.setVisible(false));this.refreshTurnOrder();}});}
  ...
  showVictory(){this.add.rectangle(640,360,1280,720,0x07040b,.84).setDepth(50);this.add.rectangle(640,360,700,440,0x171020,.99).setStrokeStyle(4,0x66e38e).setDepth(51);this.add.text(640,195,'¡VICTORIA!',{fontSize:'44px',fontStyle:'bold',color:'#79ee9c'}).setOrigin(.5).setDepth(52);const completed=SAVE.round,xp=BALANCE.xpBase+completed*BALANCE.xpPerRound,results=awardGlobalXp(xp),leveled=results.filter(x=>x.leveled);this.add.text(640,258,`Ronda ${completed} superada`,{fontSize:'22px',fontStyle:'bold',color:'#fff'}).setOrigin(.5).setDepth(52);this.add.text(640,305,`+${xp} XP global para toda la plantilla`,{fontSize:'19px',color:'#ffe596'}).setOrigin(.5).setDepth(52);const lines=leveled.length?leveled.map(x=>`${x.name}: Nv. ${x.previousLevel} → ${x.newLevel}${x.learned.length?' · Nueva habilidad':''}`).join('\n'):'Nadie ha subido de nivel todavía.';this.add.text(640,370,lines,{fontSize:'16px',color:'#ddd4e5',align:'center',lineSpacing:7,wordWrap:{width:570}}).setOrigin(.5).setDepth(52);const reward=completed%BALANCE.rewardEveryRounds===0;makeButton(this,640,510,340,66,reward?'VER RECOMPENSA':'SIGUIENTE RONDA',()=>{SAVE.round++;this.scene.start(reward?'Reward':'Placement');}).setDepth(52);}
