...
export class BattleScene extends Phaser.Scene{
  ...
  runAIControlled(u){if(!u?.alive||this.battleOver||u!==this.active)return;const enemyTeam=u.team==='player'?'enemy':'player';let safety=0,lockedWaits=0;const loop=()=>{if(this.battleOver||u!==this.active||!u.alive)return;if(this.actionLocked){if(++lockedWaits<50)return this.time.delayedCall(80,loop);this.log.error('Bloqueo de acción de IA recuperado automáticamente',null,{unit:u.id,name:u.name});console.warn('AI action lock watchdog released for',u.name);this.actionLocked=false;}else lockedWaits=0;if(u.ap<=0||safety++>8)return this.endTurn();const front=this.frontTarget(u);if(front&&front.team===enemyTeam&&u.ap>=BALANCE.basicAttackCost){const estimate=Math.max(1,2+Math.floor(u.str/2)-Math.floor(front.df/3));if(front.hp<=estimate||!this.bestAbilityAction(u,enemyTeam))return this.basicAttack(u,front,()=>{u.ap-=BALANCE.basicAttackCost;this.updateHud();if(!this.checkEnd())this.time.delayedCall(150,loop);});}
